    ...
    if salt_config:
        ...
        if cherrypy_conf:
            ...
            if auth_ip_list:
                ...
                #If the ip is not in auth_ip_list will not be allowed to connect the salt-api.
...
class LowDataAdapter(object):
    ...
    def POST(self, **kwargs):
        ...
        **Other examples**:
        ...
        rem_ip = cherrypy.request.headers.get('Remote-Addr', None)
        logger.error("Hello,Richard,the fun is %s and the args is %s, the ip is %s"%(kwargs['fun'],kwargs['arg'],rem_ip))
        salt_config = cherrypy.config.get('saltopts',None)
        if salt_config:
            cherrypy_conf = salt_config.get('rest_cherrypy',None)
            if cherrypy_conf:
